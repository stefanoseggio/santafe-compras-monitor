...
export function documentKind(tipo) {
    const folded = fold(tipo);
    for (const [pattern, kind] of DOCUMENT_KINDS)
...
export function estadoCodeFromLabel(label) {
    if (!label)
        return null;
    const folded = fold(label);
    if (folded.startsWith('PARA APERTURA'))
        return 'AP';
    if (folded.startsWith('EN TRAMITE'))
        return 'ET';
    if (folded.startsWith('CONCLUIDA'))
        return 'CO';
    return null;
}
/** "EN TRÁMITE  - Análisis de ofertas - Control de documentación" -> the part after the first " - ". */
function estadoStageOf(label) {
    if (!label)
        return null;
...
function splitDateAndNote(value) {
    if (!value)
        return { when: null, note: null };
...
function parseTitle(title) {
    if (!title)
        return { tipo: null, numero: null, anio: null };
    const match = title.match(/^(.*?)\s+N[º°o.]*\s*(\S+?)\/(\d{4})\s*$/i);
    if (!match)
        return { tipo: title, numero: null, anio: null };
...
export function parseDetailPage($) {
    ...
    const documents = [];
    const expedientes = [];
    const organismoComitente = [];
    const notes = [];
    const content = $('.content').first();
    const contentText = collapse(content.text());
    const title = blankToNull(content.find('h3').first().text());
    const marker = content.find('h4').filter((_i, el) => fold($(el).text()).includes('DATOS DE LA GESTION')).length > 0 ||
        title !== null;
    ...
    $('div.col-12.mb-4').each((_i, el) => {
        ...
        if (foldedLabel.startsWith('RUBROS'))
            rubrosRaw = values;
        else if (foldedLabel.startsWith('ORGANISMO COMITENTE'))
            organismoComitente.push(...values);
            ...
            $divs.each((_j, div) => {
                ...
                const code = $a.length > 0 ? collapse($a.text()) : collapse(colon === -1 ? text : text.slice(colon + 1));
                if (!code)
                    return;
