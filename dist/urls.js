 ...
 * `start`+`limit`.
 */
export function listingPath(query, start, limit) {
    const { filters } = query;
    const params = new URLSearchParams();
    params.set('a', 'consultas.getContrataciones');
    params.set('estado', query.estado);
    if (filters.anio !== null)
        params.set('anio', String(filters.anio));
    if (filters.objeto)
        params.set('objeto', filters.objeto);
    if (query.tipoGestion)
        params.set('tipoGestion', query.tipoGestion);
    if (filters.tipoModalidad)
        params.set('tipoModalidad', filters.tipoModalidad);
    if (filters.comprador)
        params.set('comprador', filters.comprador);
    if (filters.solicitante)
        params.set('solicitante', filters.solicitante);
    if (filters.idEspecie)
        params.set('idEspecie', filters.idEspecie);
    if (filters.idFamilia)
        params.set('idFamilia', filters.idFamilia);
    if (filters.nroGestion)
        params.set('nroGestion', filters.nroGestion);
    if (filters.nroExpediente)
        params.set('nroExpediente', filters.nroExpediente);
...
export function listingUrl(query) {
...
export function documentsUrl(idGestion) {
...
export function bidUrl(numeroExpediente) {
...
export function absoluteDocumentUrl(href) {
...
export const COMBO_PATHS = {
    ...
    subrubros: (idEspecie) => `${APP_AJAX}?a=shared.getFamiliasByEspecie&idEspecie=${encodeURIComponent(idEspecie)}`,
};
