...
describe.skipIf(process.env.CI)('live fetchListing + fetchDetail against the real Santa Fe API', () => {
    it('fetches a real listing page with well-formed items', async () => {
        const { entries } = await fetchListing(['AP'], 10, NO_SEEN, false, undefined, new Date());
    ...
    it('paginates across multiple listing pages when maxItems exceeds one page', async () => {
        const { entries } = await fetchListing(['ET'], 75, NO_SEEN, false, undefined, new Date());
    ...
    it('fetches and parses a real detail page end-to-end', async () => {
        const { entries } = await fetchListing(['AP'], 1, NO_SEEN, false, undefined, new Date());
    ...
    it('every entry from a cold run (empty seen-set) is marked is_new=true', async () => {
        const { entries } = await fetchListing(['AP'], 5, NO_SEEN, false, undefined, new Date());
        expect(entries.length).toBeGreaterThan(0);
